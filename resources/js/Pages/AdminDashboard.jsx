...
export default function AdminDashboard({ stats, recentRequests, pendingDeposits, chartData, monthlyData, activityFeed }) {
    return (
            ...
            <div className="mt-6 grid gap-6 xl:grid-cols-3">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {recentRequests.map((r) => (
                                    <tr key={r.id} className="hover:bg-slate-50">
                                        ...
                                        <td className="px-4 py-3 text-slate-600">{php(r.amount)}</td>
                ...
                <Card>
                    <CardHeader
                        ...
                        action={<Link href={route('payments.deposits.index')} className="text-sm text-sterling-green hover:underline">View all</Link>}
                    />
                    <CardBody>
                            ...
                            <ul className="space-y-3">
                                {pendingDeposits.map((d) => (
                                    <li key={d.id} className="rounded-lg border border-slate-100 p-3">
                                        ...
                                        <p className="text-xs text-slate-500">{d.bank} · {d.deposit_date}</p>
                                        <p className="mt-1 text-sm font-semibold text-emerald-700">{php(d.amount)}</p>
                                        <Link
                                            href={route('payments.deposits.show', d.id)}
                                            className="mt-1 inline-block text-xs text-sterling-green hover:underline"
                                        >
                                            Review →
                                        </Link>
                                    </li>
                        ...
                        {activityFeed.map((log) => (
                            <li key={log.id} className="flex gap-3 border-b border-slate-50 pb-3 last:border-0">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">{log.user} · {log.created_at}</p>
                                </div>
                            </li>
                        ))}
                    </ul>
